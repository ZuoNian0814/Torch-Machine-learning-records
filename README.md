# Torch-Machine-learning-records
This is my first project.

通过不断的学习，尽管我还没有找到合适的机会将深度学习与现实生活联系起来，但总体是有一些收获。我准备使用这个项目将我学习到的关于Torch框架的一些用法进行一些总结。

只是为了记录和总结我对这个框架的理解，如果有错误还请指出。

Just to record and summarize my understanding of this framework, please point out any errors.

___

## 目录 Catalogue
- 1.线性回归 | Linear Regression
- 2.多层感知机回归 | MLP Regression
- 3.多层感知机单分类 | MLP Single Classification
- 4.多层感知机多分类 | MLP Multi Classification
- 5.卷积分类 | CNN Classification
- 6.卷积回归 | CNN Regression
- 7.卷积单目标检测 | CNN Single Object Detection
- 8.卷积图像分割 | CNN Image Segmentation
- 9.卷积滑动窗口预测 | CNN Sliding Window Prediction
- 10.循环神经网络文本分类 | RNN Text Classification
- 11.循环神经网络命名实体识别 | RNN Named Entity Recognition
- 12.图神经网络图分类 | GNN Graph Classification
- 13.图神经网络节点分类 | GNN Node Classification
- 14.图神经网络节点嵌入 | GNN Node embedding
- 15.注意力机制(1) | Attention Mechanism (1)
- 16.注意力机制(2) | Attention Mechanism (2)
- 17.残差网络多层感知机 | Residual Network MLP
- 18.残差网络卷积神经网络 | Residual Network CNN
- 19.位置编码 | Positional Encoding
  
- 20.Transformer编码器 (未发布) | Transformer Encoder (Unreleased)
- 21.Transformer解码器 (未发布) | Transformer Dncoder (Unreleased)
- 22.初识多模态 (未发布) | Multimodal Preliminary (Unreleased)
- 23.特征融合方法 (未发布) | Feature-Fusion-Method (Unreleased)
- 24.多模态输入 (未发布) | Multimodal-Input (Unreleased)
